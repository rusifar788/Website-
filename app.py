...
def _word_mid_y(word) -> float:
    return (float(word[1]) + float(word[3])) / 2.0


def _word_height(word) -> float:
    return max(1.0, float(word[3]) - float(word[1]))
...
def _vertical_overlap(a, b) -> float:
    ...
    return overlap / max(1.0, min(_word_height(a), _word_height(b)))


def group_lines(page: fitz.Page) -> List[Tuple[int, List, str]]:
    """Group words into stable visual rows.

    The earlier fixed y-binning occasionally split one bank-statement row into
    two logical lines (e.g. narration separate from amount / balance). That made
    the PDF highlighter include the previous transaction row. Here we cluster by
    vertical midpoint + overlap so all words that visually belong to the same
    row stay together.
    """
    ...
    sortable = sorted(words, key=lambda w: (_word_mid_y(w), float(w[0])))
    rows = []
    for word in sortable:
        placed = False
        mid_y = _word_mid_y(word)
        for row in rows:
            row_mid = row["mid_y_sum"] / row["count"]
            tolerance = max(2.5, row["avg_height"] * 0.45)
            if abs(mid_y - row_mid) <= tolerance or any(_vertical_overlap(word, existing) >= 0.35 for existing in row["words"]):
                row["words"].append(word)
                row["mid_y_sum"] += mid_y
                row["count"] += 1
                row["avg_height"] = ((row["avg_height"] * (row["count"] - 1)) + _word_height(word)) / row["count"]
                placed = True
                break
        if not placed:
            rows.append({
                ...
                "mid_y_sum": mid_y,
                "count": 1,
                "avg_height": _word_height(word),
            })

    rows.sort(key=lambda row: min(float(w[1]) for w in row["words"]))
    result = []
    for idx, row in enumerate(rows):
        line_words = sorted(row["words"], key=lambda x: (float(x[0]), float(x[1])))
        text = " ".join(str(w[4]) for w in line_words).strip()
        result.append((idx, line_words, text))
    return result
...
def best_highlight_window(lines: List[Tuple[int, List, str]], idx: int, target_amount: float) -> Tuple[int, int]:
    """Pick the tightest contiguous line window around the chosen candidate.

    Why this exists:
    some bank PDFs split a single visual transaction into multiple extracted
    lines, and the old "scan backward to previous date line" logic could latch
    onto the prior transaction. Here we use the employee's wage amount to find
    the closest nearby amount line and highlight only the smallest sensible
    window that contains both the narration line and that amount line.
    """
    nearby = []
    for line_no in range(max(0, idx - 2), min(len(lines), idx + 3)):
        text = lines[line_no][2]
        vals = []
        for raw in re.findall(r"\d[\d,]*\.\d{2}", text):
            try:
                vals.append(float(raw.replace(',', '')))
            except Exception:
                pass
        if vals:
            best_diff = min(abs(v - target_amount) for v in vals)
            nearby.append((best_diff, abs(line_no - idx), line_no))

    if not nearby:
        return idx, idx

    best_diff, _, amount_line = min(nearby)
    if best_diff > max(250.0, target_amount * 0.02):
        return idx, idx

    start = min(idx, amount_line)
    end = max(idx, amount_line)

    # If the next immediate line looks like a wrapped continuation (no date,
    # no strong amount signal), include it, but do not spill into the next
    # transaction row.
    if end + 1 < len(lines):
        nxt = lines[end + 1][2].strip()
        has_date = line_starts_transaction(nxt)
        has_amount = bool(re.search(r"\d[\d,]*\.\d{2}", nxt))
        if not has_date and not has_amount and (end + 1 - start) <= 1:
            end += 1
...
def generate_outputs(state: dict) -> None:
    ...
    with fitz.open(str(bank_pdf)) as doc:
        ...
        for entry in state["review_queue"]:
            target_amount = float(entry.get("amount") or 0.0)
            for candidate_id in entry.get("selected_candidates", []):
                try:
                ...
                if pidx not in page_cache:
                    page_cache[pidx] = group_lines(page)
                lines = page_cache[pidx]
                if line_index < 0 or line_index >= len(lines):
                    continue

                start_line, end_line = best_highlight_window(lines, line_index, target_amount)
                line_rects = []
                for line_no in range(start_line, end_line + 1):
                    ...
                    # Small padding keeps the highlight visually clean without
                    # bleeding into neighbouring rows.
                    rect = fitz.Rect(x0 - 0.6, y0 - 0.4, x1 + 0.6, y1 + 0.4)
                ...
                if line_rects:
                    ...
                    # Draw behind the text so the bank narration stays crisp.
